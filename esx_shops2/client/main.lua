...
AddEventHandler('esx_shops2:hasEnteredMarker', function(zone)
	currentAction     = 'shop_menu'
	currentActionMsg  = ('Hmm. Olet löytänyt jotain mielenkiintoista? 🤔')
	currentActionData = {zone = zone}
end)
		...
		for k,v in pairs(Config.Zones) do
			for i = 1, #v.Pos, 1 do
				...
				if distance < Config.Size.x then
					...
					ESX.Text3D(v.Pos[i].x, v.Pos[i].y, v.Pos[i].z + 0.5 - 0.45, " Paina [~r~E~w~] avataksesi ~r~MYSTISEN~w~ valikon " )
					letSleep = false
				else
					if distance < Config.Draw2Distance then
